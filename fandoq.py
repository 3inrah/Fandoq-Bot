...
# اتصال دیتابیس به صورت سراسری (Global)
...
def show_rank(message):
    ...
    results = c.fetchall()
    text = "🏆 رده‌بندی فعلی:\n" + "\n".join([f"{i}. {name}: {score}" for i, (name, score) in enumerate(results, 1)]) if results else "هنوز امتیازی ثبت نشده!"
    bot.reply_to(message, text)

def finish_game(chat_id):
    c.execute("SELECT name, score FROM scores WHERE chat_id=? ORDER BY score DESC LIMIT 1", (chat_id,))
    winner = c.fetchone()
    bot.send_message(chat_id, f"🏁 بازی تموم شد!\n👑 قهرمان: {winner[0]} با {winner[1]} امتیاز" if winner else "🏁 بازی تموم شد!")
